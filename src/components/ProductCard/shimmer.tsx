...
export function ProductCard(props: ProductType) {
  return (
    <div className={`p-4 d-flex flex-column ${styles["product-card"]}`}>
      ...
      <div className={`d-flex ${styles.rating} mb-1 text-center`}>
        {props.isLoading ? <Skeleton circle={true} width={16} height={16} className="mx-1"/> : <img className="mr-1" src="./icons/star.svg" alt="Estrela" /> }
        {props.isLoading ? <Skeleton circle={true} width={16} height={16} className="mx-1"/> : <img className="mr-1" src="./icons/star.svg" alt="Estrela" /> }
        {props.isLoading ? <Skeleton circle={true} width={16} height={16} className="mx-1"/> : <img className="mr-1" src="./icons/star.svg" alt="Estrela" /> }
        {props.isLoading ? <Skeleton circle={true} width={16} height={16} className="mx-1"/> : <img className="mr-1" src="./icons/star.svg" alt="Estrela" /> }
        {props.isLoading ? <Skeleton circle={true} width={16} height={16} className="mx-1"/> : <img src="./icons/star-gray.svg" alt="Estrela" /> }
      </div>

      <div className={styles["product-shipping"]}>
      {props.isLoading ? <Skeleton width={`50%`}/> : 'Frete grátis' }
      </div>
